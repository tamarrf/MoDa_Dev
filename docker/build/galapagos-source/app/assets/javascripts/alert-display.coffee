class AlertDisplay
  constructor: (container, isStandalone) ->
    ...
    @_ractive = new Ractive({
      ...
      on: {
        show: () ->
          @set('isActive', true)
          window.parent.fvnl.showError(true);
          return

        hide: () ->
          @set('isDismissable', true)
          @set('isActive', false)
          window.parent.fvnl.showError(false);
          return
      }
  ...
  listenForErrors: (widgetController) ->
    ...
    # If the session (and so the ) are re-loaded, we need to clear the existing event binding first.
    # -Jeremy B April 2021
