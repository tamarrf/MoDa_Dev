class window.WidgetController
  ...
  updateWidgets: ->
    ...
    if world.ticker.ticksAreStarted()
      fvnlticks = Math.floor(world.ticker.tickCount())
      if window.parent.fvnl
        window.parent.fvnl.updateTickCount(fvnlticks);

      @ractive.set('ticks'       , fvnlticks)
...
updateWidget = (widget) ->
  ...
  switch widget.type
    ...
    when 'monitor'
      if (widget.currentValue != "N/A" and window.parent.fvnl)
        window.parent.fvnl.updateMonitor(widget.currentValue, widget.display);
