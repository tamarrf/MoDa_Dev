...
;  if count other particles-here = 1 ;; modified to be realistic, was = 1
  ...
  [
    ...
    if (candidate != nobody) and (speed > 0 or [speed] of candidate > 0) and ( particle-type != "collision")
    [
      ;; procedure to create new particles - to disable, change from "1" to "0"
      ask patch xcor ycor [
        sprout-particles 0 [
          set heading random-float 360
          set particle-type "collision"
          set speed initial-temperature
          set mass 1
          set energy (.5 * mass * speed * speed)
          set color yellow
      ...
      ;; enable command below if we want particles to rease each other
      ;; ask candidate [die]
...
to-report check-for-collision2  ;; particle procedure
  ;; Here we impose a rule that collisions only take place when there
  ;; are exactly two particles per patch.  We do this because when the
  ;; student introduces new particles from the side, we want them to
  ;; form a uniform wavefront.
...
to go-blocks
  ...
  [
    bounce
    let xcorr (xcor + dx * speed * tick-delta * 1)
    if xcorr <= -117 or xcorr >= 116 [set xcorr xcor]
    let gravity 0
    ifelse particle-type = "water" [set gravity 0.01 ][set gravity .01 ]
    
    let ycorr (ycor + dy * speed * tick-delta * 1 - gravity * (0.5 * tick-delta * tick-delta))
    setxy xcorr ycorr
    if gravity-check [ ;;how to test if it works?
      factor-gravity
    ]
    if "on" = "off" [set heading 180]
  ]
end
...
experiments-disabled
"add ink" "cool down water" "heat up water" "draw wall"
2
...
set something-cool something-cool + 1
NIL
1
T
OBSERVER
...
540
583
I just found an error in the model!
set found-error found-error + 1
NIL
1
T
OBSERVER
