...
to setup
  ...
  set-default-shape algae "lily pad"
  set-default-shape fishes "fish"
  set-default-shape bacteria "bug"
  set-default-shape nutrients "dot triangle"
  set-default-shape o2s "dot"
  set-default-shape co2s "dot square"
  
  ask patches [set pcolor 95 + 2]
...
to go
  ...
  set biggest-pop max (list (count algae) (count fishes) (count bacteria) (count dead-matters) (count o2s) (count co2s) (count nutrients))
  if biggest-pop > max-pop [
    ifelse language = "pt-br" 
      [ error (word "O modelo atingiu a população máxima permitida (" max-pop ")") ]
...
to drop-with-mouse [number]
  ...
  while [count turtles with [ pxcor = mouse-x and pycor = mouse-y] > 0][
...
to-report count-fishes
  report count fishes
end

to-report count-bacteria
  report count bacteria
end

to-report count-dead-matter
  report count dead-matters
end

to-report count-chlorophyll
  report count algae
end

to-report count-oxygen
  report count o2s
end
...
to-report count-nutrients
  report count nutrients
end
