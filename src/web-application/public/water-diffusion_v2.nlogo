...
to setup
  ...
  set region-list []
  let region 0
  foreach [-100 -80 -60 -40 -20 0 20 40 60 80] [ x ->
    foreach [ 100 80 60 40 20 0 -20 -40 -60 -80] [ y ->
      set region patches with [ pxcor >= x and pxcor < x + 20 and pycor <= y and pycor > y - 20]
      set region-list lput region region-list
    ]
  ]
...
to go
  ...
  if count particles > max-particles [
      error (word "The model has reached the particles limit (" max-particles ")")
  ]
  ask particles [
...
to color-patches
  set counter 0
  let particles-on-region 0
  foreach region-list [ region ->
      set particles-on-region particles-on region
      ifelse count particles-on-region with [particle-type = "ink"] > 0 [
        set counter counter + 1
        ask region [
          ifelse grid? [
            if remainder (pxcor) 20 != 0 and remainder (pycor) 20 != 0 [set pcolor red + 4]
          ] [
            set pcolor white
          ]
        ]
        ...
        ask region [
          if remainder (pxcor) 20 != 0 and remainder (pycor) 20 != 0 [set pcolor white]
        ]
      ]
  ]
end
