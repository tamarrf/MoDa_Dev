...
to setup
  ...
  ;https://drive.google.com/file/d/17Q1-OB1kBw9UGtpyMMzZADf1kdx5BhKT/view?usp=drive_link
  ;let image-url "https://cors-anywhere.herokuapp.com/https://moda.education/assets/images/exports/icon_experiment.png"
  ;user-message (fetch:url image-url)
  ...
  ;import-a:drawing (fetch:url image-url)
  ;import-a:pcolors (fetch:url image-url)

  ask patches [set pcolor white]

  set grid-showing? true
  set grid-color 9
  set axis-color black
  set axis-label-color red
...
to draw-grid
  create-ordered-pens 1 [
    draw-hatches
    home
    setxy -9 -9
    pendown
    set color axis-color
    set pen-size 1

    set heading  0 draw-dash setxy -9 -9
    set heading  90 draw-dash setxy -9 -9
    ;set heading  180 draw-dash home
    ;set heading  270 draw-dash home
    die
  ]

  ask patches with [pycor = -10 and pxcor >= -8 and pxcor <= 10] [
    set plabel ( word (pxcor + 9) " ")
    set plabel-color axis-label-color
  ]

  ask patches with [pxcor = -10 and pycor >= -8 and pycor <= 10] [
    set plabel ( word (pycor + 9) " ")
    set plabel-color axis-label-color
  ]
end

to draw-dash
  repeat (2 * max-pxcor) [fd 1 rt 90 fd .2 bk .4 fd .2 lt 90]
end
...
to display-labels
  ...
  if show-labels? [
    ask lpens [
      set label (word "(" (xcor + 9) ", " (ycor + 9) ")")
      set label-color axis-label-color
    ]
  ]
end
...
true
0
Polygon -7500403 true true 150 90 90 210 210 210
