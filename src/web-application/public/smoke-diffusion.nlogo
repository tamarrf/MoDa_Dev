...
to blocks-go
  ...
  create-particles 1 [
    ...
    if particle-type = "air" [
      ...
      set mass 3
      let rand 40
      set heading 0 - (rand / 2) + random (rand)
    ]
...
to blocks-set
  ...
  create-particles 2000 [
    ;; editable parameters	 
    set particle-type "air"
    ...
    if mouse-down? [
      drop-with-mouse 2000
    ]
  ]
  ask particles with [particle-type != "smoke"]
  [
    let rand 40
    if "up" = "random" [set heading random-float 360]
    if "up" = "up" [set heading 0 - (rand / 2) + random (rand)]
    if "up" = "right" [set heading 90 - (rand / 2) + random (rand)]
    if "up" = "down" [set heading 180 - (rand / 2) + random (rand)]
    if "up" = "left" [set heading 250 - (rand / 2) + random (rand)]
    set color yellow
...
to blocks-go-test
  ...
  [
    move-particles-away
    if abs [pxcor] of patch-ahead 1 >= max-pxcor - 4 or abs [pycor] of patch-ahead 1 >= max-pycor - 4
    [
    ...
    if (count collision-enemies > 0) and (collision-candidate != nobody) and (speed > 0 or [speed] of collision-candidate > 0) and ([particle-type] of collision-candidate != "ink")
    [
    ...
    set collision-flag 0
    if (count collision-enemies > 0) and (collision-candidate != nobody) and (speed > 0 or [speed] of collision-candidate > 0) and ([particle-type] of collision-candidate != "water")
    [
      set collision-flag 1
      ifelse collision-flag = 1 [
        collide-with collision-candidate
