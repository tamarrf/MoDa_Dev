...
to setup
  ...
  create-lights 1 [
    setxy 48 48
    set label word "light-intensity " light-intensity
  ]
...
to update-light-and-background
  let phase (ticks mod day-length) / day-length
  let angle 360 * phase

  ifelse use-auto-light-intensity?
    [set light-intensity ((sin (angle - 90)) + 1) / 2]  ; 0..1
    [set light-intensity manual-light-intensity / 100]
  
  ; dark gray night -> sky blue day
  let nr 60   let ng 60   let nb 60
  let dr 135  let dg 206  let db 235
  
  let r round (nr + light-intensity * (dr - nr))
  let g round (ng + light-intensity * (dg - ng))
  let b round (nb + light-intensity * (db - nb))
  
  ask patches [ set pcolor rgb r g b ]
  ask lights [ set label word "light-intensity " precision light-intensity 3]
end
...
to particle-forward
  ...
  setxy xcorr ycorr
  if abs xcorr >= max-pxcor or abs ycorr >= max-pycor and particle-type != "air"[
    die ]
end
...
to move [direction-x direction-y magnitude]
  ...
  setxy xcorr ycorr
  if abs xcorr >= max-pxcor or abs ycorr >= max-pycor and particle-type != "air"[ die ]
...
to-report count-PM
  report count particles with [particle-type = "PM"]
end

to-report count-NOx
  report count particles with [particle-type = "NOx"]
end

to-report count-ozone
  report count particles with [particle-type = "ozone"]
end

to-report count-CO
  report count particles with [particle-type = "CO"]
end

to-report count-light-intensity
  report light-intensity * 100
end
