...
//  import Blockly from 'blockly/core';
  ...
  */
 export function workspaceToSvg_(workspace, callback, customCss) {
   ...
   for (let child of clone.childNodes){
        if(child.classList.contains('blocklyDraggable')){
            for (let nestedChild of child.childNodes){
                if (nestedChild.hasChildNodes() && nestedChild.lastChild.nodeName === 'image'){
                    nestedChild.removeChild(nestedChild.lastChild);
                }
            }
        }
   }
  ...
  * @param {!Blockly.WorkspaceSvg} workspace The Blockly workspace.
  */
  export function returnWorkspaceDataUri(workspace, callback) {
    workspaceToSvg_(workspace, callback);
  }