...
export let moda_math_1quad_basic = {};
...
moda_math_1quad_basic = {
    "categories": [{
        ...
        "contents": [{
            "kind": "category",
            "name": "Background",
            "categorystyle": "background_category",
            "contents": [
                {
                    "kind": "block",
...
Blockly.defineBlocksWithJsonArray([
    ...
    // Background Blocks
    {
        ...
        "previousStatement": null,
        "nextStatement": null,
        "style": "netlogo_background_blocks",
    },
    {
        "type":"set_grid_color",
        "message0": "set grid color %1",
        "args0": [
            {
                "type": "field_dropdown",
                "name": "COLOR",
    ...
    {
        "type":"set_axis_label_color",
        "message0": "set axis label color %1",
        "args0": [
            {
                "type": "field_dropdown",
                "name": "COLOR",
...
netlogoGenerator['set'] = function (block) {
    ...
    const code = 'to blocks-set\n' +
        ...
        statement_members +
        netlogoGenerator.INDENT + 'show-grid\n' +
...
netlogoGenerator['go'] = function (block) {
    ...
    const code = 'to blocks-go\n' +
        ...
        statement_members +
        netlogoGenerator.INDENT + 'display-labels\n' +
...
// Background Blocks
netlogoGenerator['import_image_url'] = function (block) {
    const url = block.getFieldValue('URL');
    const type = block.getField('TYPE').selectedOption_[1];
    let code =
        `let image-url "${url}"\n` +
        `import-a:${type} (fetch:url image-url)\n`;
    return code;
}

netlogoGenerator['set_grid_color'] = function (block) {
    let color = block.getField('COLOR').selectedOption_[1];
    let code = 'set grid-color ' + color + '\n';
    return code;
};

netlogoGenerator['set_axis_color'] = function (block) {
    let color = block.getField('COLOR').selectedOption_[1];
    let code = 'set axis-color ' + color + '\n';
    return code;
};

netlogoGenerator['set_axis_label_color'] = function (block) {
    let color = block.getField('COLOR').selectedOption_[1];
    let code = 'set axis-label-color ' + color + '\n';
    return code;
};
