...
to mouse_click
  ...
  ask particles with [particle-type != "water"]
  [
    set mass 5
  ]
    ]
...
; Code for Unpacking: create particles
to blocks-set2
  setup

  ask particles with [who >= count turtles - recent-particles]
  [
    set color green
    
    set size 5
    if "random" = "random" [set heading random-float 360]
    if "random" = "up" [set heading 0]
    if "random" = "right" [set heading 90]
    if "random" = "down" [set heading 180]
    if "random" = "left" [set heading 270]
  ]
  reset-ticks
end
